...
const SettingsPage = () => {
  ...
  useEffect(() => {
    ...
    // Check if Google is connected (mock check for now)
    // In real app, fetch from /api/auth/google/status
  }, []);
    ...
    <div className="p-8 max-w-4xl">
      <div className="mb-8 flex justify-between items-end">
        ...
        <button 
          ...
          className={cn(
            "px-4 py-2 text-xs font-mono rounded flex items-center space-x-2 transition-colors",
            isGoogleConnected ? "bg-green-100 text-green-700 border border-green-200" : "bg-white border border-[#E4E3E0] hover:bg-[#F5F5F5]"
          )}
        >
          <div className={cn("w-2 h-2 rounded-full", isGoogleConnected ? "bg-green-500" : "bg-red-500")}></div>
          <span>{isGoogleConnected ? "GOOGLE CALENDAR CONNECTED" : "CONNECT GOOGLE CALENDAR"}</span>
        </button>
      </div>
